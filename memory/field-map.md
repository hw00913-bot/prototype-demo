...
| FLD-004 | SRC-001 | 外呼列表 | status | 任务状态 | 当前外呼任务的执行状态 | 系统根据任务执行情况自动更新 | 状态标签 | [data-anno="task-status"] | scene-list.js |
...
| FLD-025 | SRC-001 | 通话记录 | status | 通话状态 | 本次外呼的结果状态 | 系统根据外呼结果自动标记 | 状态标签 | [data-anno="call-status"] | result-records.js |
