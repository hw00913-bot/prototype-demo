...
- 仅允许使用下方源码锚点及其声明的 `fieldRefs`。权限范围以 `memory/business-rules.md` 的三角色矩阵为准：超级管理员拥有全部权限；租户管理员拥有本租户全部权限；租户运营仅能处理本租户外呼场景、统计报表、通话记录和线索记录，不能处理账号、租户、线路和标签等系统管理功能。资料不足时写入缺口说明，不得编造接口、权限、字段或业务规则。
...
- `permissionScope`：必须按当前三角色矩阵写明数据范围与功能边界；同时说明当前原型仅展示权限口径，尚未实现页面级权限拦截。
