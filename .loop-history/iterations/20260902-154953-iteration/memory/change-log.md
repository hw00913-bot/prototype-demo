...
## 2026-08-31（厚朴场景模型与账号位置补齐）

- 厚朴新建业务场景新增“大模型 / 小模型”必选项，未选择时阻止保存。
- 模型类型写入场景数据；既有三条厚朴场景补齐大模型数据，新增或编辑场景均可正确回显。
- 厚朴默认账号移动到任务关联资料之后、业务信息之前，并继续使用模拟服务端默认账号只读展示。
- 完整版说明文档升级至 v2.6，Markdown 与生成 HTML 同步；现有厚朴标注仅补充 SRC-013/FLD-056 引用并继续指向完整版说明文档，不重复维护完整规则。
- 静态资源版本更新为 `mock/data.js?v=6`、`sys-scene.js?v=5`，避免浏览器读取旧表单。

## 2026-08-31（厚朴任务详情字段收敛）

- “外呼列表 → 查看 → 任务详情”的厚朴详情移除关联方式、任务类型、服务端回调、模板、任务状态、状态获取方式和状态读取时间。
- 详情保留任务 ID、厚朴账号、任务名称、机器人、执行时段、并发、重呼、未呼优先、批次号、平台有效号码数、创建时间和外呼进度共 12 项核心信息。
- 清理 `MockHoupuTaskDetail` 中仅服务于已删除详情行的字段；业务场景配置、号码模板数据、任务卡片状态及全平台状态映射保持不变。
- 完整版说明文档升级至 v2.7 并重新生成 HTML；新增 SRC-014、T-017 和步骤 17，验收、业务规则及交接说明与页面一致。
- 厚朴详情标注仅同步 SRC-014 与 FLD-014/016/018/019，完整范围继续引用《功能说明文档》；标注数据版本升至 13，资源版本升至 `annotations.js?v=15`，Mock 资源版本升至 `mock/data.js?v=7`。
- 浏览器实测桌面与 390×844 移动端均准确展示 12 项字段，7 项非必要字段均不存在，详情顶部仍显示映射后的任务状态；控制台 0 错误、0 警告，23 个静态资源均为 200。

## 2026-09-01（电声重呼配置与 `days` 换算）

- 电声业务场景移除“最大执行天数”输入列，将间隔分钟数多选改为“统一重呼间隔（分钟）”数字输入。
- 中台按 `maxAttempts - 1` 生成统一的 `intervalMinutes` 数组，并在模拟提交电声创建任务接口前，以计划开始时间为基准，结合呼叫窗口、可呼叫星期、排除日期和 `postpone` 顺延模拟全部呼叫，换算自然日 `days`。
- 完整版说明文档升级至 v2.8，将输入、生成数组、`nextCallableTime` 顺延、自然日公式和异常拦截口径收敛到“系统管理 - 业务场景”。
- 清理不再使用的电声间隔选项 Mock 和多选样式，静态资源版本更新为 `app.css?v=4`、`mock/data.js?v=8`、`sys-scene.js?v=6`。
- 浏览器用定时启动 2026-09-02 17:30、每日 09:00–18:00、最大 4 呼、统一间隔 120 分钟实测，页面返回“预计执行 2 个自然日”；控制台 0 错误、0 警告，23 个原型静态资源全部 200。
- final 门禁要求页面锚点与正式标注的字段合同一致，因此对电声标注 ID 41、手动标注提示词和覆盖表做最小引用同步：新增 SRC-015 与 FLD-057–059，完整换算公式仍只指向《功能说明文档》；标注数据版本升至 14，资源版本升至 `annotations.js?v=16`。
